...
class studentClass:
    def __init__(self, root):
        ...
        title = Label(root, text="Manage Student Details", font=("goudy old style", 20, "bold"), bg="#033054", fg="white")
        ...
        self.course_list=["Empty"]
                #funcation _call to update the list
        self.fetch_course()
...
#==========content====
        ...
        self.CourseTable = ttk.Treeview(self.C_Frame, columns=("roll", "name", "email", "gender", "dob","contact","addmission","course","state","city","pin","adderss"), xscrollcommand=Scrollx.set, yscrollcommand=Scrolly.set)
        ...
        self.CourseTable.heading("adderss",text="Adderss")
        ...
        self.CourseTable.column("adderss",width=100)
    ...
    def add(self):
            ...
            try:
                    if self.var_roll.get()=="":
                            ...
                            row=cur.fetchone()
                            if row!=None:
                                    messagebox.showerror("Error","Roll no  AlreadyPresent",parent=self.root)
                            else:
                                    cur.execute("insert into student (roll, name, email, gender, dob,contact,addmission,course,state,city,pin,adderss) values(?,?,?,?,?,?,?,?,?,?,?,?)", (
                                            self.var_roll.get(),
                                            self.var_name.get(),
                                            self.var_email.get(),
                                            self.var_gender.get(),
            ...
            try:
                    if self.var_roll.get()=="":
                            ...
                            if row==None:
                                    ...
                                    cur.execute("UPDATE student Set  name=?, email=?, gender=?, dob=?,contact=?,addmission=?,course=?,state=?,city=?,pin=?,adderss=? where roll=?", (
                                            self.var_name.get(),
                                            self.var_email.get(),
                                            self.var_gender.get(),
                                            self.var_dob.get(),
